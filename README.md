...
| 子站收款 | 后台二选一：绑定币安**只读** API Key（注册到网关成独立账号，到账自动上榜）或直接转账（展示收款码，站长手动确认） |
...
收款码链接（可选，让施主能扫码 / 一键唤起）：币安 App → 收款 → 保存收款码图片 → 用任意二维码工具解码，得到形如 `https://app.binance.com/uni-qr/...` 的链接，填到收款设置里。本站只接受 `binance.com` 域名的链接。
