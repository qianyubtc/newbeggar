...
func TestFullFlow(t *testing.T) {
	...
	must(t, st == 200 && strings.Contains(body, "开张大吉") && strings.Contains(body, "方式一：绑定币安 API Key") && strings.Contains(body, "readonly"), "管理页: %d", st)
	// 验证码不能复用；同一 X 不能开第二个
	st, _, h = c.get("/new")
	...
	st, _, _ = c.post("/manage/payment", url.Values{"receive_link": {"https://app.binance.com/uni-qr/abc"}, "pay_id": {"UID 123456"}})
	...
	must(t, st == 200 && strings.Contains(body, "UID 123456") && strings.Contains(body, "我已施舍"), "手动付款页: %d", st)
	...
	st, _, _ = c.post("/manage/bind", url.Values{"api_key": {"k2"}, "api_secret": {"s2"}, "uid": {"90000002"}, "receive_link": {"https://app.binance.com/uni-qr/sub"}})
