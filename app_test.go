...
func newEnvWith(t *testing.T, mod func(*Config)) *env {
	...
	cfg := &Config{Listen: l.Addr().String(), BaseURL: base, DBPath: filepath.Join(t.TempDir(), "t.db"), AdminPassword: "admin-pass-123456",
		...
		RaidEnabled: true, RaidRoundSec: 600, RaidMaxBet: 50, RaidRakePct: 5, RaidBots: 6}
	if mod != nil {
		mod(cfg)
	}
	app, err := newApp(cfg)
...
func TestFullFlow(t *testing.T) {
	...
	must(t, st == 400 && strings.Contains(body, "最多两位小数"), "小数校验: %d", st)
	...
	must(t, strings.Contains(body, "排第 1 位"), "名次")
	st, body, _ = c.get("/")
	...
	must(t, st == 200 && strings.Contains(body, "123456") && strings.Contains(body, "「支付」→「转账」") && strings.Contains(body, "我已施舍"), "手动付款页: %d", st)
	...
	must(t, st == 400 && strings.Contains(body, "最多两位小数") && !strings.Contains(body, "<html"), "异步下单错误应为纯文本: %d", st)
